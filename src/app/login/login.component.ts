...
import { CheckCanActiveService } from '../Services/check-can-active.service';
...
auth2: any;
  ...
  constructor(
    ...
    private for_cookies:CookiesService,
...
this.check.if_user(this.form.email).subscribe(data=>{
  this.cookie.set('grade',btoa(data['data']));
})
this.handleToken.handleData(token.access_token);
this.AuthService.change_auth_Staust(true)
this.router.navigate([""])
this.for_cookies.setItem('logged_with',this.form.email);
